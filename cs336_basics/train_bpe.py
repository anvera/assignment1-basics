...
def read_and_split_raw_corpus(input_path: str, special_tokens: list[str]) -> list[str]:
...
def compute_pre_token_counts(corpus: list[str]) -> dict[bytes, int]:
...
def fast_train_bpe(
    ...
    """Efficiently trains a byte-pair encoding
    tokenizer on the file at the input path, with
    maximum given vocabulary size, and with the
    given list of special tokens. Outputs a tuple
    with an index of the determined vocabulary,
    as well as the list of BPE merges, the latter
